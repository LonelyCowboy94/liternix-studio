          ...
          <div className="space-y-6">
            {contactDetails.map((item, idx) => (
              <a 
                ...
                className="group flex items-center gap-6 p-4 rounded-2xl border border-transparent hover:bg-zinc-900/40 transition-all duration-300 animate-reveal opacity-0"
                style={{ animationDelay: `${0.6 + idx * 0.2}s` }}
              >
                {/* ICON CONTAINER SA BRZIM SPIN-OM NA HOVER */}
                <div className={`rotateY-card w-14 h-14 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center shadow-lg transition-all duration-500 group-hover:border-[#afff00]/50 group-hover:[transform:rotateY(360deg)] ${item.loadClass}`}>
                  {item.icon}
                </div>
                <div>
